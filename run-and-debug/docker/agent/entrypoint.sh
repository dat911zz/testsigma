#!/bin/sh

set -e  # Exit immediately if a command exits with a non-zero status

echo "Starting the build process..."

# Build jar
/opt/app/build_agent.sh

# echo "Running additional setup scripts..."

# # Run other custom setup scripts
# ROOT_FOLDER="$(
#   cd "$(dirname "$0")" || exit
#   pwd -P
# )"

# cd "$ROOT_FOLDER" || exit 1

# OS_TYPE=${OSTYPE:-"unknown"}
# OS_NAME=`uname`

# if [ -n "$ENV_TS_DATA_DIR" ]; then
#   export TS_DATA_DIR="$ENV_TS_DATA_DIR"
# else
#   if [[ "$OS_TYPE" == "darwin"* ]]; then
#     export TS_DATA_DIR="$HOME/Library/Application Support/Testsigma/Agent"
#   elif [[ "$OS_TYPE" == "freebsd"* ]] || [[ "$OS_TYPE" == "linux-gnu" ]] || [[ "$OS_NAME" == "Linux" ]]; then
#     export TS_DATA_DIR="$HOME/.testsigma/agent"
#   elif [[ "$OS_TYPE" == "cygwin"* ]] || [[ "$OS_TYPE" == "msys"* ]] || [[ "$OS_TYPE" == "win32"* ]]; then
#     echo "Unsupported OS (Windows)"
#     exit 1
#   else
#     echo "Unsupported OS"
#     exit 1
#   fi
# fi

# TS_ROOT_DIR="$ROOT_FOLDER"
# TS_AGENT_JAR_PATH="$ROOT_FOLDER"
# MAIN_JAR_FILE="$ROOT_FOLDER/agent-launcher.jar"
# LOGGING_LEVEL=INFO

# while [ $# -gt 0 ]; do
#   case "$1" in
#     --TS_DATA_DIR=*)
#       TS_DATA_DIR="${1#*=}"
#       ;;
#     --TS_ROOT_DIR=*)
#       TS_ROOT_DIR="${1#*=}"
#       ;;
#     --TS_AGENT_JAR_PATH=*)
#       TS_AGENT_JAR_PATH="${1#*=}"
#       ;;
#     --MAIN_JAR_FILE=*)
#       MAIN_JAR_FILE="${1#*=}"
#       ;;
#     --debug*)
#       LOGGING_LEVEL="DEBUG"
#       ;;
#     *)
#       printf "***************************\n"
#       printf "* Error: Invalid argument.*\n"
#       printf "***************************\n"
#       exit 1
#   esac
#   shift
# done

# if [[ "$OS_TYPE" == "darwin"* ]]; then
#   xattr -r -d com.apple.quarantine ./
# fi

# mkdir -p "$TS_DATA_DIR"

# if [ -d "$ROOT_FOLDER/jre" ]
# then
#   JAVAC="$ROOT_FOLDER/jre/bin/java"
# else
#   JAVAC="/usr/lib/jvm/jre/bin/java"
# fi

# echo "Starting the server..."
# echo "Params: $JAVAC, $TS_DATA_DIR, $LOGGING_LEVEL, $ROOT_FOLDER, $MAIN_JAR_FILE, $JAVA_OPTS_AGENT"
# echo "Run Command: $JAVAC $JAVA_OPTS_AGENT -DTS_DATA_DIR="$TS_DATA_DIR" -DTS_ROOT_DIR="$TS_ROOT_DIR" -DTS_AGENT_JAR="$TS_AGENT_JAR_PATH" -Dlogging.level.="$LOGGING_LEVEL" -cp "$ROOT_FOLDER/lib/*:$MAIN_JAR_FILE" com.testsigma.agent.launcher.Application start"
# $JAVAC $JAVA_OPTS_AGENT -DTS_DATA_DIR="$TS_DATA_DIR" -DTS_ROOT_DIR="$TS_ROOT_DIR" -DTS_AGENT_JAR="$TS_AGENT_JAR_PATH" -Dlogging.level.="$LOGGING_LEVEL" -cp "$ROOT_FOLDER/lib/*:$MAIN_JAR_FILE" com.testsigma.agent.launcher.Application start
