...
| Worker giữ token quá lâu sau khi mất lease | Run token TTL = `lease_expires_at + 60s`, gia hạn cùng lease trong CHÍNH transaction của job heartbeat (`renewRunTokenTtl`, T9/T13 — nếu không, token chết cứng ở claim+90s trong khi ngân sách chain tới 900s). Đường thu hồi ngay lập tức chỉ có MỘT call-site production: nhánh infra-requeue của `internalComplete`. Reaper **không** revoke (nó chạy role dispatch, không cầm tenant) — job bị reap được chặn bằng hàng rào epoch, và token tự hết hạn vì heartbeat gia hạn không còn khớp row nào. Test: sau requeue, gọi tiếp bằng token cũ ⇒ 401; chain 180s heartbeat đều ⇒ `complete` 200 (`run-token-ttl.test.ts`). |
