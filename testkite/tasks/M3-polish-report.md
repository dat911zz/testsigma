...
| **Áp** — đợt trả nợ M3-debt (31-08, mục 2.2) | 3 | Ba mục từng xếp hoãn, nay đã trả trong repo: NIT 73 `7864932` · NIT 12 `81e8c06` · NIT 4 `d8141aa`. Chốt sổ tài liệu: `67afbdd` · `5af5c3d` · `c68d0ff`. Plan: `plans/2026-08-31-m3-debt.md` (D1/D2/D3). |
| **Hoãn** | 0 | Sổ hoãn của M3 **đã đóng 31-08-2026**. NIT 1+14 ban đầu xếp hoãn, kiểm lại thấy `c0c2f42` đã sửa (xem 2.2). Phần **duy nhất** còn nằm ngoài repo là hạ tầng của NIT 4 (login role production tên gì, cấp bằng gì) — không phải nit code, đã thành một dòng cutover trong `M9-full-cutover-hardening.md`. |
| **Tổng** | **95** | |
...
### 2.2. Nhóm HOÃN — ĐÃ ĐÓNG SỔ 31-08-2026

Cả ba mục hoãn đã được trả trong đợt **M3-debt** cùng ngày (plan `plans/2026-08-31-m3-debt.md`,
task D1/D2/D3). Bảng dưới giữ nguyên phần *vì sao từng hoãn* để không xoá dấu vết quyết định, và
ghi ở cột cuối cái gì được trả, bằng commit nào, chứng minh bằng test nào — kèm phần **không**
trả trọn và địa chỉ mới của nó.

| # | Nội dung | Vì sao từng hoãn | Đã trả bằng gì (31-08-2026) |
|---|---|---|---|
...
| **NIT 4** ~~HOÃN~~ → **ĐÃ TRẢ (phần trong repo) + ĐÍNH CHÍNH BẤT BIẾN** | Không migration nào cấp thành viên (`GRANT testkite_dispatch TO <login role thật>`) cho `testkite_app` / `testkite_auth` / `testkite_relay` / `testkite_dispatch`; việc login role production được nạp vào các sub-role này nằm **hoàn toàn ngoài repo**. | Khoảng trống có từ M1/M2, **không do M3 tạo**. Đây là quyết định hạ tầng/ops (ai là login role, cấp bằng công cụ nào) chứ không phải code. Cấp nhầm `testkite_dispatch` cho cùng login đang giữ `testkite_app` sẽ **tái tạo đúng lỗ hổng OR-qua-kế-thừa** mà spike 2026-08-29 đã cảnh báo. | **ĐÃ TRẢ — commit `d8141aa`** (plan `plans/2026-08-31-m3-debt.md` Task D3). **Bất biến mà dòng này phát biểu là SAI và đã bị thay:** "không login role nào giữ đồng thời `testkite_app` và `testkite_dispatch`" sẽ cấm **đúng cấu hình duy nhất chạy được** — `apps/core` có MỘT `DATABASE_URL`, MỘT pool và ba lối `SET LOCAL ROLE` (`kernel/db/tenant.ts`), nên login **bắt buộc** là thành viên cả ba, không thì `SET ROLE` gãy. Cái nguy hiểm là **kế thừa**, không phải tư cách thành viên. Ba bất biến thay thế, mỗi cái có test **tự dựng vi phạm rồi dọn sạch** trên Postgres THẬT (18 ca, `apps/core/test/schema/role-separation.test.ts`): **INV-1** không login nào KẾ THỪA vai `testkite_*` (bắc cầu, xét **từng cạnh grant**) · **INV-2** không vai `testkite_*` nào là thành viên của vai khác · **INV-3** không login nào vừa giữ vai `testkite_*` vừa `SUPERUSER`/`BYPASSRLS`. Checker: `kernel/db/role-separation.ts`. Mẫu cấp + cổng kiểm: `scripts/grant-db-roles.sql` (`RAISE EXCEPTION` ⇒ psql thoát khác 0). Runbook: `../docs/runbook-db-roles.md`. **Phát hiện ngoài dự kiến:** từ PostgreSQL 16 kế thừa nằm trên **từng grant**, nên `ALTER ROLE … NOINHERIT` **KHÔNG** sửa được grant đã tồn tại — đo được: `rolinherit = false` mà vẫn đọc xuyên team. Cách sửa đúng là `REVOKE` rồi `GRANT … WITH INHERIT FALSE`. **Phần VẪN ngoài repo:** login role production tên gì, credential ra sao, cấp bằng Terraform hay tay. CI **không** chứng minh gì về cụm production (login của CI là superuser `postgres`); sự thật production chỉ đến từ `-v check_only=1` chạy trên chính cụm đó — đã ghi thành một dòng cutover trong `M9-full-cutover-hardening.md`. |
...
| **NIT 73** ~~HOÃN~~ → **ĐÃ TRẢ** | `StepOutcome` không có trường phân biệt **vòng lặp / hàng dữ liệu**, nên N lần chạy thân một `for` đều mang chung `(caseId, ordinal)`. Hệ quả cụ thể ở đường đọc: `latestStepResults` (`results-service.ts`) dùng `SELECT DISTINCT ON (step_ordinal) …` ⇒ một `for` 3 hàng dữ liệu chỉ hiện **MỘT** dòng trong report; hai hàng kia bị nuốt. | Có thật và là **mất dữ liệu ở tầng báo cáo**, nhưng sửa đúng đòi **thay đổi hợp đồng** `StepOutcome` (thêm trường phân biệt) **cộng** đổi luật đọc `DISTINCT ON` — hai phía biên, không phải một nit. | **ĐÃ TRẢ — commit `7864932`** (plan `plans/2026-08-31-m3-debt.md` Task D1). `execSeq` (khoá + thứ tự) và `loopPath` (nhãn) đi trọn tuyến runner → contract → core → DB → đường đọc; bất biến chuyển sang đường ghi bằng `res_step_results_exec_unique` (migration 0043), `DISTINCT ON` bị bỏ. **Chứng minh:** `apps/core/test/results/step-identity.test.ts` trên Postgres THẬT — *"keeps all three executions of a 3-row `for`"* (ghi 3 hàng cùng ordinal rồi đọc ra đủ **3**; cùng phép đo ấy chạy trên commit cha `74a1cf6` qua probe dựng NGOÀI repo trả về **1**, đúng chữ ký mất dữ liệu mà dòng này tố) và *"refuses a second row with the same execSeq inside one case attempt"*; phía phát: `apps/runner/test/executor/step-identity.test.ts`; biên HTTP: `apps/core/test/http/internal/complete-step-identity.test.ts` (worker CŨ không gửi `execSeq` ⇒ 200 + tái dựng theo thứ tự mảng — tương thích ngược có chủ đích vì fleet không deploy cùng nhịp với control plane; nửa vời hoặc trùng ⇒ **400 trước khi mở transaction**, có khẳng định DB không ghi nửa vời); partition tháng: `apps/core/test/results/partition.test.ts` — *"carries the new identity columns down to every partition, default included"* + *"states the step identity as a CONSTRAINT"*. **Phát hiện rộng hơn báo cáo này mô tả:** nuốt hàng KHÔNG phải bệnh riêng của `for` — step group inline lặp lại chính ordinal của group nên một case đã có ordinal trùng (1, 2, 3, 2) mà không cần vòng lặp nào, và cùng cái ordinal trùng ấy làm `sentenceIndex` trong `worker.ts` gán SAI câu NLP cho step (đã xoá). **Phần KHÔNG sửa:** fan-out cấp CASE vẫn gộp N lần chạy về một `res_case_results` — không step nào biến mất, nhưng độ mịn cấp case đòi đổi PRIMARY KEY `res_case_result_keys`, địa chỉ **M4** (đã ghi vào `M4-elements-verbs-planning.md`). |
...
NIT 73 **đã trả** ở commit `7864932`, NIT 12 ở `81e8c06`, NIT 4 (phần trong repo) ở `d8141aa` — đợt
M3-debt, plan `plans/2026-08-31-m3-debt.md` Task D1/D2/D3 ⇒ **sổ hoãn của M3 đã đóng**, chi tiết ở mục
2.2. Ba exit criteria dưới đây **không** vì thế mà đổi trạng thái: cả ba vẫn nợ host pilot đúng như đã
ghi, và phần hạ tầng của NIT 4 (login role production nào, cấp bằng công cụ gì) vẫn nằm ngoài repo —
sự thật production chỉ đến từ `scripts/grant-db-roles.sql -v check_only=1` chạy trên chính cụm đó.
...
**Verify sau đợt trả nợ M3-debt** (chốt sổ 31-08-2026, cây sạch tại `c68d0ff`): typecheck 6/6 ·
**1358 test xanh** — apps/core 740 (85 file), apps/runner 303 (25 file), run-compiler 179, contract 87,
verb-kit 12, tools 37 — `pnpm lint` sạch, `pnpm run test:tools` = 37/37 (3 file). Không mục nào giảm so
với 1300 cuối polish wave; +58 test là của D1/D2/D3.
