...
**Lớp AI: nhúng trong lõi + MCP mở cho mọi client (chốt theo hướng maintainer):**

1. **MCP server là cửa mở cho MỌI client** — MCP là chuẩn mở: Claude Desktop/claude.ai, Cursor, VS Code, agent tự viết, CI bot đều cắm được vào cùng bộ tool. Thiết kế: PAT/OAuth per-client, **scope theo token** (read-only cho bot triage, write-draft cho người soạn, không bao giờ có scope ghi non-draft), rate-limit theo token, audit log ghi client nào làm gì.
2. **AI thường trực trong lõi** — module AI trong Core API + AI job trên queue BullMQ, dùng API key *của hệ thống* với ngân sách quản trị. Tính năng v1: (a) nút sinh step từ mô tả (structured outputs ép schema, validate với catalog trước khi hiện); (b) triage chạy đêm — đọc failure + trace/screenshot Playwright, phân loại bug/locator/flake, đề xuất sửa thành draft, chạy Batch API (−50% chi phí); (c) chữa locator (thay 65 class suggestion cũ); (d) đề xuất tách step group từ chuỗi lặp.
3. **Kỹ thuật:** provider sau interface mỏng (mặc định Anthropic SDK TS, model `claude-opus-5`, $5/$25 per MTok); **prompt caching** cho prefix ổn định (catalog 35 verb + element list) — mỗi lần sinh chỉ trả token phần mô tả mới. **Guardrail:** AI chỉ ghi DRAFT, ngân sách token tháng + quota per-user + kill-switch, audit log.
4. **Mô hình 3 tầng ("không bị động"):** tầng 1 UI thuần luôn chạy — zero AI; tầng 2 AI trong lõi — ngân sách hệ thống, QA chỉ thấy nút; tầng 3 MCP mở — có PAT + client MCP là cắm não riêng. Hết token tầng 2–3 → tầng 1 nguyên vẹn.

**Worker container & câu hỏi "browser nhẹ hơn Chromium" — phán quyết (đợt workflow thứ 3, 5 agent, claim đối chiếu ngược source Java):**

- **Engine không rendering KHÔNG được chấm pass/fail — ở bất kỳ tập con nào.** 39.424/52.900 step (74,5%) cần layout/hit-testing thật (click/enter/check/selectOption = 58% tổng step chạy trên hợp đồng actionability của Playwright; họ verifyText đọc *text hiển thị* theo spec WebDriver). Soi ngược source: cả `verifyElementPresent` cũng gọi `isDisplayed()` (`VerifyElementPresenceAction.java:27`). 92% case chuỗi prereq login → ~100% case chứa step phụ thuộc rendering. Engine không layout = **pass giả** đúng lớp bug E2E sinh ra để bắt — với QA, pass giả tệ hơn test chậm.
- **Lightpanda**: README tự ghi "no graphical rendering engine"; beta 03–04/2026; benchmark 9–16× vendor tự đo chưa ai tái lập; AGPL-3.0; Playwright upstream từ chối hỗ trợ (GH #35767) → *watch, không build*, xem lại sau 12–18 tháng. **jsdom/happy-dom**: bounding rect = 0 — không phải ứng viên. **Vitest Browser Mode** là bằng chứng ngược: Vitest 4 chạy browser thật qua Playwright vì DOM giả "vỡ đúng chỗ cần hành vi thật".
- **Cái nhẹ duy nhất vừa thật vừa an toàn: `chromium-headless-shell`** — cùng Blink, ngữ nghĩa y hệt, ~35% ít RAM đỉnh (706 vs 1.094 MB), ~nửa đĩa → binary mặc định; full Chrome làm lối thoát per-case cho đuôi 1,5% verb (audit PDF/WebGL).
- **Hai vai phụ hợp lệ cho engine nhẹ** (khóa bằng schema `results.engine CHECK ('chromium')`, output khác vào `advisory_signals`): cổng pre-flight ~5s trước run đêm (env sập → 1 abort sạch thay vì 3.111 false-fail + 10–15GB trace rác); đường đọc-DOM (linkedom/cheerio) cho AI/MCP/chữa locator — ứng viên chữa phải được worker Chromium xác nhận lại.
- **Blueprint v1:** 1 host 16 vCPU/32GB — 6 worker batch + 1 interactive (docker-compose tĩnh), mỗi worker: Node → BullMQ concurrency 4 → 1 headless-shell sống lâu → 4 context = **24 context song song**; đơn vị job = **chuỗi case** (~1.550–2.000 job/đêm, session login truyền tự nhiên trong context); regression đêm ~1,1–2,2h; fail assertion = job hoàn thành verdict failed, chỉ lỗi hạ tầng mới retry; non-root + sandbox bật + seccomp, worker zero-credential, `executeJavaScript` qua `page.evaluate` 5s có audit; trace chỉ giữ khi fail; ~$150–300/tháng.
- **Cổng tuần-0 trước khi mua hạ tầng:** (1) census có tính lặp prereq login không (±45% giờ-context); (2) port 50 chuỗi đo histogram giây/verb thật; (3) app đích + pool ≥24 tài khoản chịu được 24 phiên song song — *thứ dễ giết kế hoạch nhất, không phải container*. Bonus cho spec: ngữ nghĩa verb vendor tự mâu thuẫn (IfElement "VISIBLE" chỉ `findElement()`, không `isDisplayed()`) → đường E phải tái đặc tả từng verb, không port theo tên.
