...
- **Ba role Postgres (bổ sung 28-08-2026):** `testkite_app` (đường request, RLS lọc theo `app.team_id`), `testkite_relay` (outbox), `testkite_auth` (**chỉ SELECT** trên `api_tokens`/`memberships`/`users`/`idn_oidc_connectors`/`idn_oidc_login_states`, policy `auth_lookup`). Role thứ ba tồn tại vì RLS fail-closed đúng thiết kế: khi chưa biết tenant thì không tra được credential — vòng lặp này chỉ gỡ được bằng một role có policy riêng, KHÔNG phải bằng `BYPASSRLS`.
