...
- Results 5→3 tầng: job_runs → res_case_results (iteration nuốt bảng data-driven; attempt; **engine CHECK — chỉ engine có layout thật ghi verdict**) → res_step_results (+failure_context JSON ≤32KB; **danh tính step trong một attempt = `exec_seq`** — 1-based, dày, theo thứ tự thực thi, UNIQUE ở đường ghi — kèm `loop_path` int[] ghi vị trí vòng lặp; **đọc theo `exec_seq`, KHÔNG theo `step_ordinal`**: một `for` N hàng dữ liệu sinh N dòng thật, ordinal trùng nhau là chuyện thường, kể cả khi không có vòng lặp nào — step group inline lặp lại ordinal của group). res_artifacts theo attempt; res_advisory_signals (không có cột verdict — structural).
...
- **Bốn role Postgres (bổ sung 28-08-2026; role thứ tư thêm ở M3):** `testkite_app` (đường request, RLS lọc theo `app.team_id`), `testkite_relay` (outbox), `testkite_auth` (**chỉ SELECT** trên `api_tokens`/`memberships`/`users`/`idn_oidc_connectors`/`idn_oidc_login_states`/`orc_run_tokens`, policy `auth_lookup`), `testkite_dispatch` (**SELECT+UPDATE `job_runs` XUYÊN team**, policy `dispatch_all USING (true)`). Hai role sau tồn tại vì RLS fail-closed đúng thiết kế: khi chưa biết tenant thì không tra được credential, và đường claim của dispatcher cũng chưa biết tenant nên không lọc được theo `app.team_id` — vòng lặp này chỉ gỡ được bằng một role có policy riêng, KHÔNG phải bằng `BYPASSRLS`. **Cấp đúng + cổng kiểm: [`../testkite/docs/runbook-db-roles.md`](../testkite/docs/runbook-db-roles.md) + `testkite/scripts/grant-db-roles.sql`** — policy PERMISSIVE bị OR nên quyền của hai role cộng dồn, giữ được cách ly là nhờ 3 bất biến: **INV-1** không login nào KẾ THỪA vai `testkite_*` (xét **từng cạnh grant**, bắc cầu — từ PG16 inherit nằm trên grant nên `ALTER ROLE … NOINHERIT` KHÔNG sửa grant cũ, phải `GRANT … WITH INHERIT FALSE`); **INV-2** không vai `testkite_*` nào là thành viên của vai `testkite_*` khác (ở đây `WITH INHERIT FALSE` KHÔNG đủ — `SET ROLE` vẫn với tới hợp nhất quyền); **INV-3** không login nào vừa giữ vai `testkite_*` vừa `SUPERUSER`/`BYPASSRLS`.
- **Quota 6 chỉ số, 5 điểm cưỡng chế** (enqueue 429 chỉ cho API/UI — fan-out schedule miễn, đậu lại ở hàng đợi `job_runs` của Postgres; dispatch cap; metering 60s; pre-PUT artifact với trần 2GB/run + sampling theo failure-signature; pre-flight AI). Chạm trần: chặn cái mới, **run đang bay luôn chạy nốt**.
...
- **Dispatcher (một spec duy nhất):** `job_runs` pending trên Postgres (`FOR UPDATE SKIP LOCKED`) → deficit-weighted RR, cost = clamp(ceil(steps/10),1,8), cap/team = clamp(ceil(fleet×share×1.5),4,floor(fleet×0.5)), sàn chống đói 60s, fan-out 200/tick, dead-man alert + fallback FIFO 120s.
